package ru.zorb.services.meta.impl.loader;
...
import ru.zorb.services.meta.bean.MetaFieldMutableBean;
...
public class DynamicContentLoader implements MetaLoader {
   ...
   * (non-Javadoc)
   *
   * @see ru.zorb.services.meta.MetaLoader#loadClasses()
   */
  @Override
   ...
   * (non-Javadoc)
   *
   * @see ru.zorb.services.meta.MetaLoader#loadFields()
   */
  @Override
  public Collection<MetaFieldMutable> loadFields(Map<String, MetaClassMutable> clazzCode, Map<String, MetaClassMutable> clazzId) {
    DynamicContent dcTable = DynamicContent.DYNAMIC_CONTENT;
    ru.zorb.storage.jooq.tables.DynamicField dfTable = ru.zorb.storage.jooq.tables.DynamicField.DYNAMIC_FIELD;
    Result<Record> records = dslContext.select().from(dfTable).innerJoin(dcTable).on(dcTable.ID.equal(dfTable.DYNAMIC_CONTENT_ID)).where(dcTable.FIXED.equal(false)).fetch();
    if (records == null) {
    ...
    for (Record field : records) {
      //Линкованный класс
      MetaClass linkedMetaClass = clazzId.get(String.valueOf(field.get(dfTable.LINK_TO_DYNAMIC_CONTENT, Long.class)));
      ...
      MetaFieldMutableBean dynamicFieldBean = new MetaFieldMutableBean(field.get(dfTable.ID).toString(), metaClassById, field.get(dfTable.NAME), field.get(dfTable.CODE),
          field.get(dfTable.COLUMN_FIELD), FieldType.getFieldType(field.get(dfTable.TYPE1)), linkedMetaClass, backReferenceField, null);
