...
import ru.zorb.security.dynamiccontent.ContentSecurityManager;
import ru.zorb.services.meta.*;
import ru.zorb.services.meta.bean.MetaClassMutableBean;
import ru.zorb.services.meta.impl.GetObjectsParams;
import ru.zorb.services.meta.impl.SortOrder;
import ru.zorb.services.meta.staticcontent.MetaClassConditionChecker;
import ru.zorb.services.meta.staticcontent.meta.MetaClassStaticContent;
import ru.zorb.storage.jooq.tables.FieldView;
import ru.zorb.storage.jooq.tables.daos.DynamicContentDao;
import ru.zorb.storage.jooq.tables.daos.DynamicFieldDao;
...
public class MetaClassDataSourceImpl implements ContentDataSource<MetaClass> {
   ...
   * (non-Javadoc)
   *
   * @see ru.zorb.services.meta.DataSource#setMetaInitializer(ru.zorb.services.meta. MetaInitializer)
   */
  @Override
   ...
   * (non-Javadoc)
   *
   * @see ru.zorb.services.meta.DataSource#createObject(java.lang.String, java.util.Map)
   */
  @Override
   ...
   * (non-Javadoc)
   *
   * @see ru.zorb.services.meta.DataSource#updateObject(java.lang.String, java.lang.String, java.util.Map)
   */
  @Override
  @CacheEvict(cacheNames = { "metaObjList", "metaObj", "metaObjListCond", "metaObjCnt", "fieldObjList", "fieldObj", "fieldObjListCond", "fieldObjCnt" }, allEntries = true)
	...
	private void update(MetaClass content) {
		...
		if (!isFieldEquals(content.getName(), oldContent.getName())) {
			updateSetMoreStep = recordUpdateSetFirstStep
					.set(DSL.field(ru.zorb.storage.jooq.tables.DynamicContent.DYNAMIC_CONTENT.NAME), content.getName());
		}
		if (!isFieldEquals(content.isCheckObjectSecurity(), oldContent.isCheckObjectSecurity())) {
			Field<Boolean> field = DSL.field(ru.zorb.storage.jooq.tables.DynamicContent.DYNAMIC_CONTENT.CHECKOBJECTSECURITY);
		...
		if (!isFieldEquals(content.isCheckSecurity(), oldContent.isCheckSecurity())) {
			Field<Boolean> field = DSL.field(ru.zorb.storage.jooq.tables.DynamicContent.DYNAMIC_CONTENT.CHECKSECURITY);
			if (updateSetMoreStep == null) {
		...
		if (updateSetMoreStep != null) {
			updateSetMoreStep
					.where(DSL.field(ru.zorb.storage.jooq.tables.DynamicContent.DYNAMIC_CONTENT.ID).eq(Long.valueOf(content.getId())))
					.execute();
		}
		metaInitializer.init();
	}
   ...
   * (non-Javadoc)
   *
   * @see ru.zorb.services.meta.DataSource#deleteObject(java.lang.String, java.lang.String)
   */
  @Override
  @Transactional
  @CacheEvict(cacheNames = { "metaObjList", "metaObj", "metaObjListCond", "metaObjCnt", "fieldObjList", "fieldObj", "fieldObjListCond", "fieldObjCnt" }, allEntries = true)
   ...
   * (non-Javadoc)
   *
   * @see ru.zorb.services.meta.DataSource#getCntObjList(java.lang.String)
   */
  @Override
  ...
  public Collection<MetaClass> getObjects(GetObjectsParams params) {
    ...
    LinkedHashMap<MetaField, ru.zorb.services.meta.impl.SortOrder> orderFields = params.getOrderFields();
    Stream<MetaClass> stream = metaClasses.stream();
    ru.zorb.services.meta.conditions.Condition condition = params.getCondition();
    if (condition != null) {
